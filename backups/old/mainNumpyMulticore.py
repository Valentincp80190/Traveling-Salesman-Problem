    ...
    for line in file:
        ...
        if i > 5:
            ...
            try:
                ...
                #row = pd.DataFrame([[float(line[1]),float(line[2]),len(nodes)]], columns=["x","y","root"])
                #print(row)
                nodes = np.append(nodes, np.array([[float(line[1]),float(line[2]),len(nodes)]]), axis=0)
            except IndexError:
                break
            #nodes = pd.concat([nodes, row], axis=0, ignore_index=True)
        i = i + 1
    file.close
    print("Temps de chargement du fichier : " + str(time() - currentTime) + ".")
    return True

def loadFullGraphFromNode(node):
    global arcs
    r, c = nodes.shape

    for i in range(r):
        if node == i : continue
        if node in arcs[:, 0] and i in arcs[:, 1]: continue
        cost = math.sqrt(math.pow(nodes[node,0] - nodes[i,0], 2) + math.pow(nodes[node,1] - nodes[i,1], 2))
        arcs = np.append(arcs, np.array([[node,i,cost]]), axis=0)
    return True
...
def loadFullGraphArcs():
    ...
    nodesList = list(np.arange(r))
    #print(nodesList)

    while len(nodesList) > 0 :
        currentNode = nodesList.pop()
        x = threading.Thread(target=loadFullGraphFromNode, args=(currentNode,))
        x.start()
        #loadFullGraphFromNode(currentNode)
