...
arcsListe = []
class Arc():
    def __init__(self, _idS, _idE, _cost):
        self.idS = _idS
        self.idE = _idE
        self.cost = _cost
...
def loadFullGraphArcs():
    ...
    for i in range(rN) :#nombre d'arcs = n(n-1)/2
        for j in range(rN) :
            if i == j : continue
            if j in arcs[:, 0] and i in arcs[:, 1]: continue
            cost = math.sqrt(math.pow(nodes[i,0] - nodes[j,0], 2) + math.pow(nodes[i,1] - nodes[j,1], 2))
            
            #row = pd.DataFrame([[i,j,cost]], columns=["idS","idE","cost"])
            #row = np.array()
            #arcs = pd.concat([arcs, row], axis=0, ignore_index=True)
            #arcs = np.append(arcs, np.array([[i,j,cost]]), axis=0)
            arcsListe.append(Arc(i,j,cost))
